...
type Darts struct {
    ...
    KeyString2IntMap map[string]int // 存储字面值 - ID(int) 的映射
...
func (d *dartsBuild) resize(newSize int) {
    if newSize > cap(d.darts.Base) {
        ...
        d.darts.TermList = append(d.darts.TermList, make([]map[int]bool, (newSize-len(d.used)))...)
